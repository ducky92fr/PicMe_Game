...
class Game {
  ...
  demoGame = () => {
    ...
    setTimeout(() => {
      setInterval(this.arrayImageUpdate, 3000);
    }, 7000);
    setTimeout(() => {
      setInterval(this.changeSourcePictureArea, 3000);
    }, 7000);
    setTimeout(() => {
      setInterval(this.addImages, 3000);
    }, 7000);
  };
  ...
  addImages = () => {
    const markup = `
      ...
      <img class = 'image' src=${
        currArrayImages[getRandomInt(currArrayImages.length - 1)]
      } alt=''>
      </div>`;
  ...
  playerSelectImage = event => {
    ...
    if (trackImageClicked === srcImgGauche) {
      ...
      this.updateUI();
      event.target.src =
        "https://www.onlygfx.com/wp-content/uploads/2018/09/4-comic-check-mark-1.png";
      audioTrue.play();
    } else {
      audioFalse.play();
    }
  ...
  createGrid = () => {
    ...
    board.style.setProperty(
      ...
      "repeat(" + this.level[this.currLevel] + ", 1fr)"
    );
    board.style.setProperty(
      "grid-template-rows",
      "repeat(" + this.level[this.currLevel] + ", 1fr)"
  ...
  updateNodeList = () => {
    ...
    for (let i = 0; i < imagePictureArea.length; i++) {
      imagePictureArea[i].onclick = this.playerSelectImage;
    }
  };
  trackBestPlayer = () => {
    const player = {
    ...
    let UpdatedArrayBestPlayer = JSON.parse(
      localStorage.getItem("trackBestPlayer")
    );
  ...
  topPlayer = () => {
    ...
    if (arrayTopPlayer != null) {
      const numberToLoop =
        arrayTopPlayer.length >= topPlayer.length ? 5 : arrayTopPlayer.length;
